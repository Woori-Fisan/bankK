...
* **핵심 코드**:
```java
// platform/backend — 단말기 JWS 서명 검증 (global/util/CryptoUtil)
public static String verifyJwsAndGetPayload(String jwsString, String publicKeyPem) {
    ...
    if (jwsObject.verify(new RSASSAVerifier(publicKey))) {
        return jwsObject.getPayload().toString(); // 검증 성공 → 페이로드 반환
    }
    return null; // 검증 실패
}
 
// platform/backend — Replay Attack 방지 (domain/auth/service/AuthService)
...
```
* **코드 링크**: [CryptoUtil.java](https://github.com/Woori-Fisan/bankK/blob/develop/platform/backend/src/main/java/com/woorifisan/platform/global/util/CryptoUtil.java)
---
...
// 3. AES 키(CEK)를 은행 RSA 공개키(RSA-OAEP-256)로 래핑
const exportedAesKey = await window.crypto.subtle.exportKey('raw', aesKey);
const encryptedKey = await window.crypto.subtle.encrypt(
  { name: 'RSA-OAEP' }, bankPublicKey, exportedAesKey
);
// 4. JWE Compact Serialization 조립 후 반환
return { reqPayload: `${header}.${encKey}.${ivStr}.${cipherStr}.${tagStr}`, aesKey };
```
* **코드 링크**: [bankCrypto.ts](https://github.com/Woori-Fisan/bankK/blob/develop/platform/frontend/src/utils/bankCrypto.ts)
---
